...
int main(void) {
  ...
  // Configure SPI pins (SCK: PA5, MISO(SDI): PB4, MOSI(SDO): PB5, CE:PA11)
...
initTIM(TIM15);
  ...
  // Set PA5, PB4, and PB5 to high
  initSPI(7, 0, 1);  // BR = 3 (fPCLK/16), CPOL = 0, CPHA = 1
  initDS1722();
  ...
  while(1) {
    ...
    */
    ...
    while(inString(request, "\n") == -1) {
      // Wait for a complete request to0 be transmitted before processing
  ...
  float temperature = readTemperature();
        ...
        printf(temperatureStr, "Temperature: %.2f°C", temperature);
    ...
    sendString(USART, "Debug: Webpage sent\n");
